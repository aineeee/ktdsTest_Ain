package com.ktdsuniversity.edu.todo.controller; //건들면 안됨
...
public class TodoController {
	...
	public static void main(String[] args) {
		...
		if(menu == 1) {
			...
			TodoVO todo = service.read(key);//서비스에 key를 줘서 받아 옴. / 있다 없다 판단
			if(todo == null) {
				...
				if(todo.isSuccess()) {
					...
					if(yn.equalsIgnoreCase("Y")) {
						isSuccess = service.update(key,false); // isSuccess 에 할당
					...
					String yn = scan.nextLine();
					if(yn.equalsIgnoreCase("Y")) {
						isSuccess = service.update(key, true); // isSuccess에 할당
					}
				}
			if(isSuccess) {
